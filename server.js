...
// Serve static frontend files from /public (only in non-serverless mode)
// In Vercel, static files are served directly, not through Express
...
if (process.env.MONGO_URL) {
  ...
  console.warn('MONGO_URL not set, MongoDB connection skipped (may be in serverless mode)');
}
...
// Socket.io setup - only if not in serverless mode
let server;
let io;
if (require.main === module) {
  // Only create HTTP server and Socket.io when running directly (not serverless)
  ...
  // Serverless mode - create a mock io object
  io = {
    to: () => ({ emit: () => {} })
  };
}
...
app.get('/api/me', auth, async (req, res) => {
  try {
    ...
    const user = await User.findById(req.userId).select(
      'username favoriteClub badges reputation createdAt'
    );
...
app.post('/api/post', auth, upload.single('media'), async (req, res) => {
  ...
  if (req.file && req.file.path) {
    ...
    try {
      ...
      console.warn('media processing failed', e && e.message);
      thumb = uploadedWebPath;
    }
  }
  ...
  if (me && (!me.badges || !me.badges.includes('first_post'))) {
    me.badges = me.badges || [];
    me.badges.push('first_post');
    await me.save();
  }
...
app.get('/api/post/:id/comments', async (req, res) => {
  const comments = await Comment.find({ post: req.params.id, removed: { $ne: true } })
    .populate('user', 'username')
    .sort({ createdAt: 1 })
    .limit(500);
  res.json(comments);
...
app.post('/api/comment/:id/report', auth, async (req, res) => {
  ...
  try {
    await createNotification(comment.user, 'comment:reported', {
      commentId: comment._id,
      by: req.userId,
      reason
    });
  } catch (e) {
    console.warn('Failed to create report notification:', e.message);
  }
...
app.get('/api/moderation/comments', auth, async (req, res) => {
  ...
  const items = await Comment.find({ reportCount: { $gt: 0 } })
    .populate('user', 'username')
...
app.post('/api/like/:id', auth, async (req, res) => {
  ...
  try {
    ...
    console.warn('reputation update failed', e && e.message);
  }
...
app.post('/api/user/:id/follow', auth, async (req, res) => {
  ...
  if (String(target._id) !== String(req.userId)) {
    ...
    try {
      await User.findByIdAndUpdate(target._id, { $inc: { reputation: 2 } });
      await awardBadges(target._id);
    } catch (e) {
      console.warn('Failed to update reputation on follow:', e.message);
    }
  }
  res.json({
...
app.get('/api/user/:username', async (req, res) => {
  const u = await User.findOne({ username: req.params.username }).select(
    'username favoriteClub badges reputation followers following createdAt'
  );
...
app.post('/api/me/username', auth, async (req, res) => {
  ...
  const u = await User.findByIdAndUpdate(req.userId, { username: wanted }, { new: true }).select(
    'username favoriteClub badges reputation'
  );
  res.json(u);
});
...
app.post('/api/match/:id/event', async (req, res) => {
  ...
  const ev = await MatchEvent.create({
    matchId,
    eventType,
    minute: minute || 0,
    playerIds: playerIds || [],
  ...
  try {
    io.to('match:' + matchId).emit('match:event', ev);
  } catch (e) {
    console.warn('emit event failed', e && e.message);
  }
  ...
  events.forEach(e => {
    ...
    if (String(e.team).toLowerCase() === 'home') home += xg;
    else away += xg;
    entries.push({
      minute: e.minute,
      timestamp: e.timestamp,
      xgHome: Number(home.toFixed(4)),
      xgAway: Number(away.toFixed(4)),
      winProbHome: Number(computeWinProb(home, away).toFixed(4))
    });
  });
  try {
    io.to('match:' + matchId).emit('match:xg', { entries });
  } catch (e) {
    console.warn('emit xg failed', e && e.message);
  }
...
app.get('/api/match/:id/xg', async (req, res) => {
  const events = await MatchEvent.find({ matchId: req.params.id }).sort({
    minute: 1,
    createdAt: 1
  });
  let home = 0,
...
app.post('/api/match/:id/prediction', auth, async (req, res) => {
  const matchId = req.params.id;
  const { question, choices, expiresInSec } = req.body || {};
  ...
  try {
    io.to('match:' + matchId).emit('prediction:new', p);
  } catch (e) {
    console.warn('Failed to emit prediction:new:', e.message);
  }
...
app.post('/api/prediction/:pid/submit', auth, async (req, res) => {
  ...
  if (p.submissions.find(s => String(s.user) === String(req.userId)))
    return res.status(409).json({ error: 'already_submitted' });
  p.submissions.push({ user: req.userId, choice, createdAt: new Date() });
  await p.save();
  try {
    io.to('match:' + p.matchId).emit('prediction:update', {
      id: p._id,
      counts: p.submissions.reduce((acc, s) => {
        acc[s.choice] = (acc[s.choice] || 0) + 1;
        return acc;
      }, {})
    });
  } catch (e) {
    console.warn('Failed to emit prediction:update:', e.message);
  }
...
app.get('/api/prediction/:pid', async (req, res) => {
  ...
  if (!p) return res.status(404).json({ error: 'not_found' });
  const counts = p.submissions.reduce((acc, s) => {
    acc[s.choice] = (acc[s.choice] || 0) + 1;
    return acc;
  }, {});
...
app.post('/api/prediction/:pid/resolve', auth, async (req, res) => {
  const pid = req.params.pid;
  const { correctChoice } = req.body || {};
  const p = await Prediction.findById(pid).populate('submissions.user');
  if (!p) return res.status(404).json({ error: 'not_found' });
  ...
  if (String(p.creator) !== String(req.userId) && requester.role !== 'admin')
  ...
  try {
    io.to('match:' + p.matchId).emit('prediction:resolved', { id: p._id, correctChoice, awarded });
  } catch (e) {
    console.warn('Failed to emit prediction:resolved:', e.message);
  }
...
app.get('/api/leaderboard', async (req, res) => {
  try {
    ...
    const top = await User.find()
      .sort({ reputation: -1 })
      .limit(20)
      .select('username reputation favoriteClub badges');
    res.json(top);
  } catch (err) {
...
// Minimal homepage so visiting / in a browser shows a friendly page
// Serve a minimal single-page app so the site is interactive from the browser.
// Serve index.html from /public when root requested (only in non-serverless mode)
if (require.main === module) {
  app.get('/', (req, res) => {
